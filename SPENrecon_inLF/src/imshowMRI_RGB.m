function imshowMRI_RGB(image_matrix, scale, tile_shape, titles,cmap)
%
...
image_matrix=reshape(image_matrix,size(image_matrix,1),size(image_matrix,2),3,[]);
...
if isempty(tile_shape)
    tile_shape = [1 size(image_matrix,4)];
end

if (prod(tile_shape)>=size(image_matrix,4))
    image_matrix=cat(4,image_matrix,zeros(size(image_matrix,1),size(image_matrix,2),3,prod(tile_shape)-size(image_matrix,4)));
else
    error('image tile rows x columns must larger than the 3rd dim extent of image_matrix');
end

if ~isempty(titles),
    assert(numel(titles) == size(image_matrix,4), 'number of titles must equal 3rd dim extent of image_matrix');
end
