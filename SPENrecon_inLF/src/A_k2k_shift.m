function [A] = A_k2k_shift(Q,k_source,k_response,dy)
...
for m = 1 : M
    for n =1: N
        if Q==0
            ...
            A(m,n) = sum(FTQuadratic_shift(Q,linspace(k_source(n)-k_response(m)-deltak/2,k_source(n)-k_response(m)+deltak/2,256),dy));
        end
    end
end
