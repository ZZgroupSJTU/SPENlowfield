function xtSel            = sigselhsvd_LYL(xt, params, optWat, optLip, optMeta)
...
dt                    = params.dt;
%% set default values
[params, optWat, optLip, optMeta] = initParams(params, optWat, optLip, optMeta);
for indr = 1:Nr
...
switch params.signalType
    ...
    case 'meta'
        indSel    = length(indLip)+length(indWat)+1:length(indLip)+...
                        length(indWat)+length(indMeta);
    otherwise
        indSel    = indWat;
end
if ~isempty(indSel)
